...
from sqlalchemy import (
    Column, Integer, String, Float, DateTime, Boolean, ForeignKey, Text, func
)
from sqlalchemy.orm import relationship
from datetime import datetime

# Import the declarative Base from your database connection file.
# All your models must inherit from this Base.
...
class Supplier(Base):
    ...
    # This creates the one-to-many relationship back to Product
    products = relationship("Product", back_populates="supplier")

class Product(Base):
    """
    Represents a specific product in the Walmart catalog that is tracked by VeriCart.
    """
    __tablename__ = "products"

    id = Column(String, primary_key=True, index=True)
    # The public-facing unique identifier for a product (e.g., SKU or GTIN).
    product_id_str = Column(String, unique=True, index=True, nullable=False)
    name = Column(String, nullable=False)
    category = Column(String, index=True) # e.g., "Electronics", "Pharmaceuticals"
    supplier = Column(String, index=True)

    # A product can have many scans and many alerts associated with it.
...
class Scan(Base):
    """
    Represents a single scan event for a product, either from the supply
    chain or a customer.
    """
    __tablename__ = "scans"

    id = Column(String, primary_key=True, index=True)
    # The server sets the timestamp to ensure consistency across timezones.
    timestamp = Column(DateTime(timezone=True), server_default=func.now())
    latitude = Column(Float, nullable=False)
    longitude = Column(Float, nullable=False)
    user_id = Column(String, index=True) # ID of the customer or employee
    is_anomaly = Column(Boolean, default=False, nullable=False)
    scan_order = Column(Integer, default=1) # A counter for this product's scans

    # Foreign key linking this scan to a specific product.
    product_pk = Column(Integer, ForeignKey("products.id"), nullable=False)
    
    # Relationships for easy navigation in SQLAlchemy.
    product = relationship("Product", back_populates="scans")
    # A scan can trigger at most one alert.
    triggered_alert = relationship("Alert", back_populates="triggering_scan", uselist=False, cascade="all, delete-orphan")


class Alert(Base):
    """
    Represents a fraud alert generated by the system when an anomaly is detected.
    """
    __tablename__ = "alerts"

    id = Column(Integer, primary_key=True, index=True)
    timestamp = Column(DateTime(timezone=True), server_default=func.now())
    alert_type = Column(String, nullable=False) # "Velocity", "Geographic", "Cluster"
    risk_score = Column(Float)
    status = Column(String, default="new", nullable=False, index=True) # "new", "investigating", "resolved"
    notes = Column(Text) # For analyst investigation notes.

    # Foreign key linking this alert back to the product it's for.
    product_pk = Column(String, ForeignKey("products.id"), nullable=False)
    # Foreign key linking this alert to the specific anomalous scan that triggered it.
    triggering_scan_pk = Column(Integer, ForeignKey("scans.id"), nullable=False, unique=True)
...
class ProvenanceEntry(Base):
    ...
    status = Column(String, nullable=False) # e.g., 'Manufactured', 'In-Transit', 'In-Store'
    location = Column(String, nullable=False)
    timestamp = Column(DateTime, default=datetime.utcnow)
    handler = Column(String) # e.g., 'SupplierA', 'LogisticsCo', 'Walmart Store #1234'
